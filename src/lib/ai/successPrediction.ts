...
async function generateAIAssessment(projectId: string): Promise<ProjectSuccessPredictionInput> {
  try {
    ...
    return JSON.parse(response.choices[0].message.content) as ProjectSuccessPredictionInput;
  } catch (error) {
...
function preprocessAssessmentData(input: ProjectSuccessPredictionInput): tf.Tensor {
...
export async function predictProjectSuccess(projectId: string): Promise<SuccessPredictionResult> {
  try {
    ...
    Our ML model has predicted a ${(successProbability * 100).toFixed(1)}% probability of success for this startup.
    ...
    return {
      ...
      keyStrengths: ["Strong concept", "Market opportunity", "Potential for growth"],
      keyRisks: ["Execution challenges", "Market competition", "Funding requirements"],
      valuationEstimate: {
        min: 500000,
        avg: 1000000,
        max: 2000000
      },
      suggestedFocus: ["Product development", "Market validation", "Team building"]
    };
  }
}